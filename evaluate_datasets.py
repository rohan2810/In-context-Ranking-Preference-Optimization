...
def get_reddit(csv_path: str, max_movies: int = 20000, silent: bool = False, split: str = 'train', num_negtive: int = 1) -> Dict[str, Dict[str, Union[str, List[Tuple[int, int]]]]]:
    """
    ...
    """
    print('Loading Reddit dataset... from ', csv_path)
    ...
    positive_samples = [
        # list(dict.fromkeys([remove_year_from_movie_name(movie) for movie in post if movie in movie2id])) 
    ...
    for i in tqdm.tqdm(range(len(positive_samples)), desc='Processing Reddit Data', disable=silent):
        ...
        num_rnd = num_mv - len(ground_truth) - len(filtered_llm_rec)
        random_movies = []
        if num_rnd > 0:
            # we want more movies
        ...
        combined = ground_truth_ranked + llm_rec_ranked + random_ranked

        # combined = [(movie, rank) for movie, rank in combined if movie.strip()]
        if combined:
            movies, relevance_rank = [], []
            for _ in range(num_negtive):
                combined_movies, combined_ranking = zip(*combined)
                movies.append("|||".join(combined_movies))
                relevance_rank.append(list(combined_ranking))
                random.shuffle(combined)
            prompt = f"\nUser: {context}\nSystem: "
            data[prompt].update({"movies": movies, "relevance_rank": relevance_rank})
        else:
            print("Combined tokens in dataloader", combined)
            raise NotImplementedError
    return data

def get_nlp(csv_path: str, max_movies: int = 20000, silent: bool = False, split: str = 'train', num_negtive: int = 1) -> Dict[str, Dict[str, Union[str, List[Tuple[int, int]]]]]:
    """
    Load and preprocess the Reddit dataset, and return a dict of prompts and responses
    containing movies and their rankings.
    ...
    for i in tqdm.tqdm(range(len(relevant_items)), desc='Processing Reddit Data', disable=silent):
...
def get_dataset(name: str, split: str, silent: bool = False, cache_dir: str = None, embed_dir: str = None, num_negtive: int = 1):
    """Load the given dataset by name. Supported by default are 'shp', 'hh', and 'se'."""
    if name == 'reddit':
        # data_dir = 'datasets/reddit_test_with_gpt3.5_output.csv' if split == 'eval' else 'datasets/reddit_train_with_gpt3.5_output.csv'
        data = get_reddit(embed_dir, 20000, split=split, silent=silent, num_negtive=num_negtive)
    elif name == 'nlp':
        # data_dir = 'datasets/reddit_test_with_gpt3.5_output.csv' if split == 'eval' else 'datasets/reddit_train_with_gpt3.5_output.csv'
        data = get_nlp(embed_dir, 20000, split=split, silent=silent, num_negtive=num_negtive)
    else:
...
def tokenize_batch_element(prompt: str, chosens: str, truncation_mode: str, tokenizer,
...
def get_batch_iterator(names: List[str],
                       ...
                       num_negtive: int = 1) -> Iterator[Dict]:
    ...
    with TemporarilySeededRandom(seed):
        ...
        for name in names:
            ...
            for prompt, data in get_dataset(name, split, silent=silent, cache_dir=cache_dir, embed_dir=embed_dir, num_negtive=num_negtive).items():
    ...
    while True:
        ...
        for prompt, movies, relevance_rank, truncation_mode in flat_data:
            ...
            batch_elements = tokenize_batch_element(
            ...
            for idx, batch_element in enumerate(batch_elements):
                batch_element['relevance_rank'] = relevance_rank[idx]
            batch.extend(batch_elements)
            example_idx += 1 
        # ─── CHANGE START ─────────────────────────────────────────────────
        # Instead of checking `== batch_size`, check `>= batch_size` and slice off exactly batch_size.
            while len(batch) >= batch_size:
                to_yield = batch[:batch_size]
                batch    = batch[batch_size:]
                yield collate_fn(to_yield)
