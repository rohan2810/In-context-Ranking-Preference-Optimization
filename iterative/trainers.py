    ...
    def train(self):
        ...
        for batch in self.train_iterator:               
            ...
            for microbatch_idx in range(self.config.gradient_accumulation_steps):
                ...
                try:
                    with torch.no_grad():
                        formatted_prompt, movies, relevance_rank = global_microbatch["prompt"], global_microbatch['movies'], global_microbatch['relevance_rank']
                        print("=" * 80 + "\n\n", global_microbatch.keys())
                        print("=" * 80 + "\n\n", global_microbatch['prompt'])
                        print("=" * 80 + "\n\n", global_microbatch['movies'])
                        print("=" * 80 + "\n\n", global_microbatch['relevance_rank'])
                        policy_samples, reference_samples = self.get_batch_samples(global_microbatch)
                        for prompt, sample in zip(global_microbatch['prompt'], policy_samples):
                            policy_text_table.append({"prompt": prompt, "generation": sample})
                        if self.config.loss.name in {'dpo', 'ipo', 'rdpo'}:
                            for prompt, sample in zip(global_microbatch['prompt'], reference_samples):
                                reference_text_table.append({"prompt": prompt, "generation": sample})
                        print("=" * 80 + "\n\n", policy_samples)
                        print("=" * 80 + "\n\n", reference_samples)

                        new_batch = []
                        for p, s, m, r in zip(formatted_prompt, policy_samples, movies, relevance_rank):
                            if 'llama' in self.config.model.name_or_path.lower():
                                items = [i.split(".", 1)[-1].strip().lower().strip('\'').strip('.') for i in s.split('\n\n')[-1].strip().split('\n')]
                            elif 'phi' in self.config.model.name_or_path.lower():
                                items = [i.split(".", 1)[-1].strip().lower().strip('[').strip(']').strip('\'') for i in s.split('System:')[-1].strip().split('\n')]
                            else:  # gemma
                                parsed = s.split(".", 1)[-1].strip().lower()
                                items = [i.strip().strip('\'') for i in parsed.split(',')]
                            
                            m = [i.strip('.').lower() for i in m]
                            print("*" * 80 + "\n\n", items)
                            print("*" * 80 + "\n\n", m)
                            new_movie = [item for item in items if item in m]
                            new_relevance_rank = [r[m.index(item)] for item in new_movie if item in m]
                            if len(new_movie) > 0:
                                batch_element = tokenize_batch_element(
                                    prompt=p,
                                    chosen="|||".join(new_movie),
                                    truncation_mode='keep_start',
                                    tokenizer=self.tokenizer,
                                    max_length=512,
                                    max_prompt_length=1024
                                )
                                batch_element['relevance_rank'] = new_relevance_rank
                                new_batch.append(batch_element)
                                global_microbatch = collate_fn(new_batch)
                                global_microbatch = move_batch_to_device(global_microbatch, device=self.rank)
                            else:
                                continue

                    if len(global_microbatch) == 0:
                        continue
